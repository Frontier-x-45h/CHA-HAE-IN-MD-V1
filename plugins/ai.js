...
cmd({
    ...
    try {
        ...
        if (!response || !response.data || !response.data.result) {
            return reply("🙋 Sorry Love No response received from the chahaein API. Please try again later.");
        }
        ...
        if (!gptResponse) {
            return reply("🤦‍♀️Sorry hun but The API returned an unexpected format. Please try again later.");
        }

        // Image AI à envoyer
        const ALIVE_IMG = 'https://files.catbox.moe/xo91yg.jpg; // Remplacez par l'URL de votre image AI

        // Légende avec des informations formatées
        const formattedInfo = `💋 *Queen's Response:*\n\n${gptResponse}`;
        ...
        console.error("Ouch sorry but error in GPT command:", error);
        ...
        const errorMessage = `
🤦‍♀️An error occurred while processing the GPT command.
🛠 *Error Details*:
${error.message}
...
cmd({
    ...
    try {
        ...
        // AI image to attach
        const AI_IMG = 'https://files.catbox.moe/xo91yg.jpg'; // Replace with a valid image URL
        ...
        await conn.sendMessage(from, {
            ...
            contextInfo: { 
                ...
                forwardedNewsletterMessageInfo: {
                    newsletterJid: '120363398176647533@newsletter',
                    newsletterName: 'CHA HAE-IN-MD 𝐀𝐈',
                    serverMessageId: 143
                }
            }
        }, { quoted: mek });
