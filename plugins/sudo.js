...
cmd({
    ...
    if (!isOwner) return reply("🤦🏻‍♀️Baka Only the owner can add a sudo user.");

    let number = q || (quoted ? quoted.sender : null);
    if (!number) return reply("🤦🏻‍♀️Baka Please provide a number or reply to a message.");
...
cmd({
    ...
    if (!isOwner) return reply("🤦🏻‍♀️Baka Only the owner can remove a sudo user.");

    let number = q.replace(/[^0-9]/g, ""); // Clean the number
    if (!number) return reply("🤦🏻‍♀️Baka Please provide a number.");

    if (!sudoUsers.includes(number)) return reply("🤦🏻‍♀️Baka This user is not a sudo.");
...
cmd({
    ...
    if (sudoUsers.length === 0) return reply("🤦🏻‍♀️Baka No sudo users registered.");
