...
async (conn, mek, m, { from, args, isOwner, reply }) => {
    if (!isOwner) return reply("*📛 sorry senpai ᴏɴʟʏ ᴛʜᴇ ᴏᴡɴᴇʀ ᴄᴀɴ ᴜsᴇ ᴛʜɪs ᴄᴏᴍᴍᴀɴᴅ!*");
...
async (conn, mek, m, { from, args, isOwner, reply }) => {
    ...
    if (args[0] === "on") {
        ...
        return reply(`*🫟 ᴇxᴀᴍᴘʟᴇ:  .ᴀᴜᴛᴏsᴇᴇɴ ᴏɴ*`);
    }
}); 
//--------------------------------------------
...
cmd({
    ...
    if (["warn", "delete", "kick", "off"].includes(action)) {
        ...
        return reply("🤦🏻‍♀️ *Bakaka that's an Invalid option!* Use *antilink warn/delete/kick/off*.");
    }
});
cmd({
    on: "body"
    ...
    if (isUrl(body)) { // Using isUrl to detect links
        ...
        return reply(`⚠️ *Admins turned on antilink hence Warning! Links are not allowed here.*`);
...
cmd({
    ...
    if (!isBotAdmins) return reply("*_baka I'm not an admin, so I can't take action!_*");
...
cmd({
  ...
  try {
    if (!isGroup) {
      return reply("Baka this command can only be used in groups.");
    }
...
cmd({
  ...
  try {
    if (!isGroup) {
      return reply("🤦🏻‍♀️baka This command can only be used in groups.");
    }
...
cmd({
  ...
  try {
    ...
    if (!isGroup) return reply('🤦🏻‍♀️Baka This command can only be used in a group.');
    ...
    if (args[0] === "on") {
      ...
      await reply("🤦🏻‍♀️Baka Anti-link feature is now enabled in this group.");
    } else if (args[0] === "off") {
      config.ANTI_LINK = "false";
      await reply("Anti-link feature is now disabled in this group.");
    } else {
      await reply(`*🤦🏻‍♀️Baka Invalid input! Use either 'on' or 'off'. Example:antilink on*`);
    }
  } catch (error) {
...
cmd({
    ...
    try {
        ...
        if (!isGroup) return reply("🤦🏻‍♀️Baka 𝐓𝐡𝐢𝐬 𝐅𝐞𝐚𝐭𝐮𝐫𝐞 𝐈𝐬 𝐎𝐧𝐥𝐲 𝐅𝐨𝐫 𝐆𝐫𝐨𝐮𝐩❗");
        ...
        if (!isBotAdmins) return reply("🤦🏻‍♀️Baka 𝐏𝐥𝐞𝐚𝐬𝐞 𝐏𝐫𝐨𝐯𝐢𝐝𝐞 𝐌𝐞 𝐀𝐝𝐦𝐢𝐧 𝐑𝐨𝐥𝐞 ❗");
        ...
        return reply(`*🧕🏻oky senpai Here is your group invite link:*\n${inviteLink}`);
...
cmd({
  ...
  try {
    ...
    if (!isAdmins) return reply("🤦🏻‍♀️Baka You need to be an admin to broadcast in this group!");

    if (!q) return reply("🤦🏻‍♀️Baka Provide text to broadcast in all groups!");
...
cmd({
    ...
    if (!isAdmins) return reply("🤦🏻‍♀️Baka You must be an admin to use this command.");
    if (!isBotAdmins) return reply("🤦🏻‍♀️Baka I need to be an admin to change the group profile picture.");
...
cmd({
    ...
    if (!isOwner) return reply("*🤦🏻‍♀️Baka Only my senpai can use this command!*");

    const cmdList = `
    ----------------------------------------
        CHA HAE-IN-MD SETTINGS 
    -----------------------------------------
...
cmd({
    ...
    if (!isOwner) return reply("*🤦🏻‍♀️Baka ᴏɴʟʏ ᴛʜᴇ ᴏᴡɴᴇʀ ᴄᴀɴ ᴜsᴇ ᴛʜɪs ᴄᴏᴍᴍᴀɴᴅ!*");
