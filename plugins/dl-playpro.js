...
cmd({
  ...
  try {
    ...
    if (!response.data || !response.data.status || !response.data.output) {
      return reply('❌ Lol my bad but im Unable to fetch the song. Please try again later.');
    }
    ...
    await reply(`🎵 *Hold a sec while l fetch your song love ${songDetails.title}...*`);

    // Download the song
    const songResponse = await axios.get(downloadUrl, { responseType: 'arraybuffer' });
    if (!songResponse.data) {
      return reply('❌ im unperfect l Failed to download the song. Please try again later.');
    }
    ...
    await conn.sendMessage(from, {
      ...
      caption: `> © Gᴇɴᴇʀᴀᴛᴇᴅ ʙʏ ✦CHA HAE-IN-MD✦`,
      contextInfo: {
        ...
        forwardedNewsletterMessageInfo: {
          ...
          newsletterName: '『 CHA HAE-IN-MD tech🪀 』',
          serverMessageId: 143
        }
      }
    }, { quoted: mek });
