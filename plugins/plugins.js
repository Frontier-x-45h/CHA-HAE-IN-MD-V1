...
const REPO_OWNER = 'Frontier-x';
const REPO_NAME = 'Frontier';
...
async (conn, mek, m, { from, reply }) => {
    try {
        ...
        let pluginList = "📂 *CHA HAE-IN-MD Plugins:*\n\n";
...
async (conn, mek, m, { from, reply, args, senderNumber }) => {
    try {
        ...
        if (m.quoted && m.quoted.key.fromMe) {
            ...
            if (/📂 \*CHA HAE-IN-MD Plugins:\*/i.test(quotedText)) {
        ...
        const statusMessage = {
            image: { url: `https://files.catbox.moe/xo91yg.jpg` }, // Replace with your image URL
