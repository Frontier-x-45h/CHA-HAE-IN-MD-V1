  ...
  async (bot, message, senderInfo, { from, args, reply }) => {
    try {
      ...
      if (!userSessions[from]) {
        return reply("❌ 🤦🏻‍♀️Baka You do not have an active temporary email. Use `.tempmail new` to generate one.");
      }
      ...
      if (action === 'read') {
        ...
        if (!emailID) {
          return reply("❌ 🤦🏻‍♀️Baka Provide an email ID. Example: `.tempmail read 12345`");
        }
        ...
        if (!emailData || !emailData.mail_subject) {
          return reply("❌ 🤦🏻‍♀️Baka Invalid email ID or non-existent email.");
        }
      ...
      // Option invalide
      return reply("❌ 🤦🏻‍♀️Baka lnvalid option. Use `.tempmail new`, `.tempmail inbox`, or `.tempmail read <ID>`");
