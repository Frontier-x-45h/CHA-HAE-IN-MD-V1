...
cmd({
    ...
    try {
        if (!q) {
            return reply("❌ 🤦🏻‍♀️Baka provide the title and artist, separated by a comma.\nExample: `.lyrics faded, Alan Walker`");
        }
        const parts = q.split(",");
        if (parts.length < 2) {
            return reply("❌ 🤦🏻‍♀️Baka provide both the title and the artist, separated by a comma.");
        }
...
cmd({
    ...
    try {
        if (!q) {
            return reply("❌ 🤦🏻‍♀️Baka provide the song title.\nExample: `.lyrics Faded`");
        }
